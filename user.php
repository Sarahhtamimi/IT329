...
$photo = !empty($user['photoFileName']) ? "images/" . $user['photoFileName'] : "images/Avatar.png";

//REQ3 retriving total recipes+likes
$totalRecipesQuery = "SELECT COUNT(*) AS totalRecipes FROM recipe WHERE userID = $userID";
$totalRecipesResult = mysqli_query($conn, $totalRecipesQuery);
$totalRecipesRow = mysqli_fetch_assoc($totalRecipesResult);
$totalRecipes = $totalRecipesRow['totalRecipes'];
...
$categoryQuery = "SELECT * FROM recipecategory";
...
//REQ5
//+counting num of likes for each recipe
$allRecipes = [];

if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['category']) && $_POST['category'] != 'all') {
    ...
    $allRecipesQuery = "
        ...
        LEFT JOIN likes l ON r.id = l.recipeID
        WHERE rc.categoryName = '$selectedCategory'
        GROUP BY r.id
    ";
} else {
    $allRecipesQuery = "
...
$allRecipesResult = mysqli_query($conn, $allRecipesQuery);
//if request is GET → all recipes
//if request is POST and category selected → only recipes of that category


while ($row = mysqli_fetch_assoc($allRecipesResult)) {
    $allRecipes[] = [
        "id" => $row['id'],
        "name" => $row['NAME'],
...
<!-- REQ2 displaying welcome note + User information -->

    <div class="top-row">
      <h2>Welcome, <span id="userFirstName"><?php echo htmlspecialchars($firstName); ?></span>!</h2>
    </div>
  ...
  <div class="profile-card">
    ...
    <div class="profile-left">
      <img id="infoPhoto" class="avatar" src="<?php echo htmlspecialchars($photo); ?>" alt="Profile photo">
    </div>
  ...
  <span id="infoFirst"><?php echo htmlspecialchars($firstName); ?></span>
  <span id="infoLast"><?php echo htmlspecialchars($lastName); ?></span>
</h4>

      <div class="profile-meta">
  <div class="meta-row">
    <span class="meta-icon">📧</span>
    <span id="infoEmail"><?php echo htmlspecialchars($email); ?></span>
  </div>
</div>
  ...
  <!--REQ3 My recipes link + DISPLAYING totals-->
<section class="section-box my-recipes-box">
  <div class="my-recipes-row">
    ...
    <div class="totals">
<span>Total recipes: <span id="totalRecipes"><?php echo $totalRecipes; ?></span></span>
<span>Total likes: <span id="totalLikes"><?php echo $totalLikes; ?></span></span>
    </div>
  </div>
</section>
...
<form id="categoryForm">
  <div class="mood-pills" id="categoryPills">
    <button 
      ...
      name="category" 
      value="all"
      class="pill <?php if ($selectedCategory == 'all') echo 'active'; ?>">
      All
    </button>
    ...
    <?php foreach ($categories as $cat): ?>
      <button 
        ...
        class="pill <?php if ($selectedCategory == $cat) echo 'active'; ?>">
...
<?php } else { ?>
  <p style="margin-top:14px; font-weight:600; color:#725C3A;">No recipes found in this category.</p>
<?php } ?>


    </section>

    <!--REQ6 Favourite recipes -->
   <section class="section-box">
  <h3 class="fav-title">My Favourite Recipes <span class="fav-heart">❤️</span></h3>
  ...
  <?php } else { ?>
    <p style="margin-top:14px; font-weight:600; color:#725C3A;">You do not have any favourite recipes yet.</p>
  <?php } ?>
</section>
  ...
  var allRecipes = <?php echo json_encode($allRecipes); ?>;
...
renderFavourites();
  ...
  var categoryButtons = document.querySelectorAll("#categoryPills .pill");
...
for (var i = 0; i < categoryButtons.length; i++) {
  categoryButtons[i].addEventListener("click", function () {
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4 && xhr.status === 200) {
        allRecipes = JSON.parse(xhr.responseText);
        renderAll();
      }
    };
